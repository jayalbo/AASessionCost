...
exports.handler = async (event, context) => {
  ...
  const { appId, callId, startTime, endTime, multiHost = false, customerId, customerSecret } = event.queryStringParameters || {};
  let speakers = [];
  let totalCost = 0;
  ...
  try {
    ...
    if (!allCallInfo || allCallInfo.length === 0) {
      return {
        statusCode: 404,
        headers,
        body: JSON.stringify({ error: "No call data found for the specified parameters" })
      };
    }
